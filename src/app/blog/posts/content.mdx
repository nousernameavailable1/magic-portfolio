...
Replace the `public/images/profilepic.png` file with your own avatar image. It is used on the `/about` page as well as on the dynamically generated `open-graph` images.
...
Replace the content in the `src/resources/content.tsx` file with your own content. You can use custom components in most cases, since the props are usually declared as `ReactNode` instead of `string`, but it's important that you need to import them in the file.
...
Social links are rendered on the `/about` page and in the footer based on the array configured in the `social` object. You can set custom icons for each, but don't forget to register them in `src/resources/icons.ts`. Read the [Once UI icon documentation](https://once-ui.com/docs/icons) for more information.
